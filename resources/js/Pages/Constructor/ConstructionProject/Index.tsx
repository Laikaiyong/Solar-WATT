...
export default function ProjectList({ auth, projects }: ProjectListProps) {
    ...
    const getStatusClasses = (status: string) => {
        switch (status) {
            case 'pending':
                return 'bg-yellow-100 text-green-800 border border-green-300 rounded-full px-2 py-1 text-xs'; // Smaller size
            case 'in_progress':
                return 'bg-blue-100 text-red-800 border border-red-300 rounded-full px-2 py-1 text-xs'; // Smaller size
            case 'completed':
                return 'bg-green-100 text-yellow-800 border border-yellow-300 rounded-full px-2 py-1 text-xs'; // Smaller size
            default:
                return '';
        }
    };
